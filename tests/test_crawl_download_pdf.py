...
class TestCrawlAndDownloadPDF(unittest.TestCase):
    ...
    def test_crawl_and_download_pdf(self, mock_get, mock_get_user_agent):
        ...
        expected_filepath = os.path.join(
            output_folder, "The structure of an Amyloid Precursor Protein_tali.pdf"
        )
        self.assertTrue(
            os.path.isfile(expected_filepath), f"{expected_filepath} does not exist"
        )

        self.assertGreater(
            os.path.getsize(expected_filepath), 0, f"{expected_filepath} is empty"
        )
    ...
    @patch(
        "biorxiv.crawl_download_pdf.get_random_user_agent", return_value="test-agent"
    )
    @patch("biorxiv.crawl_download_pdf.requests.Session.get")
    def test_crawl_and_download_pdf_no_pdfs(self, mock_get, mock_get_user_agent):
        ...
        self.assertEqual(
            len(os.listdir(output_folder)), 0, "No files should have been created"
        )
